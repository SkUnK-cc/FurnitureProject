...
class FragmentOther: BaseAddTransFragment(), View.OnClickListener {
    ...
    private var vm : FragmentOtherVM? = null
...
//    private var keyboardUtil: KeyboardUtil? = null
    ...
    override fun initView(rootView: View) {
        vm = ViewModelProviders.of(this).get(FragmentOtherVM::class.java)
...
//        }
    ...
    private fun refreshTypePickerIfNeed(){
        ...
        mPvOptions?.dismiss()
        goodsList.clear()
        goodsList.addAll(DbHelper.getDetailTypeManager().getDetailTypeList(DetailTypeBean.TYPE_PAY_OTHER))
...
//                }
    ...
    private fun isValidate(): Boolean {
        ...
        if(count.isNotEmpty() && !DigitUtil.isInteger(count)){
            ToastUtil.showShort(tv_count.text.toString()+"格式错误,只能为整数")
            return false
        }
    ...
    override fun updateTrans() {
        ...
        vm?.updateTrans(et_price?.text.toString(), et_count?.text.toString(),et_note.text.toString(),time!!)
    }
}